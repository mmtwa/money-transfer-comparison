...
const CurrencyWidget = ({ 
  ...
  }, [initialAmount, initialFromCurrency, initialToCurrency]);

  // Notify parent component when our internal state changes
  useEffect(() => {
    if (onCurrencyChange) {
      onCurrencyChange(amount, fromCurrency, toCurrency);
    }
  }, [amount, fromCurrency, toCurrency, onCurrencyChange]);
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white rounded-lg shadow-lg p-6" style={{
      background: 'white',
      boxShadow: '0 4px 20px rgba(0, 0, 0, 0.1)',
    }}>
      <h2 className="text-xl font-semibold mb-6 text-center">Compare Money Transfers</h2>

      {/* You're sending section */}
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">You're sending</label>
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">They're receiving</label>
        <div className="relative">
          <button
      ...
      <p className="text-base text-gray-600 mb-6 text-center">We'll check all providers for the best deal</p>

      <button
        onClick={handleSearchClick}
        disabled={isLoading}
        className="w-full text-white py-4 px-4 rounded-md flex items-center justify-center"
        style={{
