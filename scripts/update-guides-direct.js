/**
 * Combined script to update guide content from React components and fix guide status
 * with a direct MongoDB connection
 * 
 * Usage: node scripts/update-guides-direct.js
 */
...
// IMPORTANT: Hardcode MongoDB connection string to avoid batch file issues
let MONGODB_URI = 'mongodb+srv://mercedesclkexj:eey6HPOhWY8xNV0H@cluster0.dbtnsfv.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0';
// Use environment variable if available (but prefer hardcoded for reliability)
if (process.env.MONGODB_URI && process.env.MONGODB_URI.includes('mongodb')) {
  MONGODB_URI = process.env.MONGODB_URI;
}

console.log('Using MongoDB URI:', MONGODB_URI.replace(/mongodb:\/\/([^:]+):([^@]+)@/, 'mongodb://USER:PASSWORD@'));
  ...
  try {
    ...
    for (const guidePath of guidePaths) {
      ...
      if (!content) {
        ...
        // Use findOneAndUpdate with upsert instead of new Content and save
        ...
        createdCount++;
        console.log(`Created content for ${guidePath}`);
      } else {
        ...
        content.images = {
          heroImage: content.images?.heroImage || heroImage,
        ...
        await content.save();
        updatedCount++;
        console.log(`Updated content for ${guidePath}`);
      }
    }
    
    console.log(`\nMigration complete. Created ${createdCount} new guides, updated ${updatedCount} existing guides.\n`);
    return true;
  } catch (error) {
...
// -------------------- PART 2: FIX GUIDE STATUS --------------------

// Run the fixes for guide status and images
const runFixes = async () => {
  console.log('\n----- STARTING GUIDE FIXES -----\n');
